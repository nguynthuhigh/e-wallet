...
export default function VoucherForm() {
  ...
  const validate = () => {
    let tempErrors = {};
    if (voucherData.discountValue > 100 && voucherData.type === "discount_percent") tempErrors.discountValue = "Discount value is less than or equal to more than 100";
    if(voucherData.code.match(/\s/) ) tempErrors.code="Invalid Code Voucher"
  ...
  const handleSubmit =async (e) => {
    ...
    try {
      ...
      console.log(error)
      setErrors({erroSubmit:error.response.data.message})
      setIsLoading(false)
    }
  };
  ...
  return (
    ...
    <form className="p-5 max-w-[800px]  w-full mx-auto bg-white shadow rounded-[15px]" onSubmit={handleSubmit}>
      <div className='font-semibold text-4xl my-2'>Add new voucher</div>
      ...
      <div className='font-semibold text-center text-red-500'>{errors.erroSubmit}</div>
