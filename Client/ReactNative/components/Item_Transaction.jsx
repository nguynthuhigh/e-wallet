...
const Item_Transaction = ({ index, item, id }) => {
  return (
    ...
    <View
      className={index % 2 === 0 ? "bg-white h-[85px]" : "bg-gray-50 h-[85px]"}
    >
      <View className="flex-row items-center  p-4">
        <View className="ml-[25px]">
          <Text className="font-medium text-[16px] ">
            {convertTitle(id, item?.sender, item?.receiver)}
          </Text>
          <Text className="text-[12px] font-light ml-[-3] mt-2">
            {" "}
            {formatTime(item?.createdAt)}{" "}
          </Text>
          <Text className="text-[12px] font-light ml-[-3] mt-1">
            {" "}
            {item?.message}{" "}
          </Text>
        </View>
        <View className="ml-auto mt-auto">
          <Text className="text-[16px] font-semibold">
            {format.formatCurrency(
              item.amount,
              item?.currency?.symbol ? item?.currency?.symbol : "VND"
            )}
          </Text>
        </View>
      </View>
